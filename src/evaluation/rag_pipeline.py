...
- Deduplicación de sources para evitar repeticiones en consola, JSON y web
"""
...
def _deduplicate_sources(sources: list) -> list:
    """Elimina fuentes duplicadas manteniendo el orden."""
    seen = set()
    unique = []
...
def answer_with_rag(query: str, vector_store, k: int = 4) -> tuple:
    ...
    candidates = _deduplicate_docs(candidates)
...
def answer_with_rag_multimodal(
    ...
    relevant_captions = []
    visual_keywords = [
        "diagrama", "figura", "tabla", "imagen", "gráfico",
